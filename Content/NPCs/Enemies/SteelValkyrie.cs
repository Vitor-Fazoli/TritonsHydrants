...
using GearonArsenal.Content.Items.Weapons.Ranged;
using System.IO;
...
namespace GearonArsenal.Content.NPCs.Enemies
{
    public class SteelValkyrie : ModNPC
    {
        ...
        {
            return ModContent.NPCType<ValkyrieBot>();
        }
        ...
        private void SpawnMinions()
        {
            ...
            {
                int index = NPC.NewNPC(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<ValkyrieBot>(), NPC.whoAmI);
                ...
                if (minionNPC.ModNPC is ValkyrieBot minion)
                {
        ...
        private void CheckSecondStage()
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (otherNPC.active && otherNPC.type == MinionType() && otherNPC.ModNPC is ValkyrieBot minion)
                {
        ...
        private void DoFirstStage(Player player)
        {
            ...
            if (FirstStageDestination != LastFirstStageDestination)
            {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    //Dust.QuickDustLine(NPC.Center + toDestinationNormalized * NPC.width, FirstStageDestination, toDestination.Length() / 20f, Color.Orange);
                    Player p = Main.player[NPC.target];

                    Projectile.NewProjectile(new EntitySource_TileBreak(2, 2), NPC.position, new Vector2(5, 5), ProjectileID.WoodenArrowHostile,
                        10, 0f, default);
                    NPC.position -= NPC.netOffset;
                }
            }
