...
namespace TritonsHydrants.Content.Items.Weapons
{
	...
	public class AbyssalFlag : ModProjectile
	{
		...
		private void NormalAI() {
			// Gravidade constante da parábola
			Projectile.velocity.Y += 0.35f; 

			Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.ToRadians(90f);

			// Mantenha seu código de Dust original aqui...
			...
			if (Main.rand.NextBool(4)) {
				Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, ModContent.DustType<Sparkle>(), 0, 0, 254, Scale: 0.3f);
