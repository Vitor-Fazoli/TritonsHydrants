    ...
    public static class ModSabres {
        ...
        public static bool HoldItemManager(Player player, Item item, int slashProjectileID, Color chargeColour = default(Color), float slashDelay = 0.9f, float ai1 = 1f, Action<Player, bool> customCharge = null, int delaySpeed = 4) {
            ...
            if (player.itemAnimation > 0) {
                ...
                if (onAttackFrame) {
                    ...
                    if (Main.myPlayer == player.whoAmI) {
                        ...
                        int p = Projectile.NewProjectile(new EntitySource_TileBreak(2,2),
                    ...
                    if (item.shoot <= ProjectileID.None && player.itemTime == 0) { player.itemTime = item.useTime; item.beingGrabbed = false; }
                }
            ...
            if (player.itemTime > 0) {
                ...
                if (player.velocity.Y == 0) { delaySpeed /= 2; }

                // cap
                delaySpeed = Math.Max(delaySpeed, 1);

                // Reset if swinging
                if (player.itemAnimation > 0) { player.itemTime = Math.Max(player.itemTime, item.useTime); } else if (Main.myPlayer == player.whoAmI) {
                ...
                if (internalChargeTicker >= delaySpeed) { internalChargeTicker = 0; }

                // delay item time unless at 0
                if (internalChargeTicker > 0) { player.itemTime++; }
                internalChargeTicker++;

                // flash and correct
                if (player.itemTime <= 1) {
                    player.itemTime = 1;
                    if (customCharge != null) { customCharge(player, true); } else { PlayerFX.ItemFlashFX(player, 45, new PlayerFX.SoundData(25) { volumeScale = 0.5f }); }
                }
            }

            // HACK: allows the player to swing the sword when held on the mouse
            if (Main.mouseItem.type == item.type) {
                if (player.controlUseItem && player.itemAnimation == 0 && player.itemTime == 0 && player.releaseUseItem) { player.itemAnimation = 1; }
            }
        ...
        public static bool IsChargedShot(Player player) {
            ...
            if (player.itemAnimation == player.itemAnimationMax - 1) { return true; }
        ...
        public static void OnHitFX(Player player, Entity target, bool crit, Color colour, bool glow = false) {
            ...
            Dust d = Dust.NewDustPerfect(target.Center - dir * 30f,
                DustID.Ash, dir * 15f, 0, colour, (crit ? 1.5f : 1f));
            d.noLight = glow;
        }
        ...
        public static bool AIDashSlash(Player player, Projectile projectile, float dashFrameDuration, float dashSpeed, int freezeFrame, ref Vector2? dashEndVelocity) {
            ...
            if ((int)projectile.ai[0] < dashFrameDuration) {
                ...
                if (player.velocity.Y == 0) { player.velocity = new Vector2(0, (projectile.velocity * dashSpeed).Y); } else { player.velocity = new Vector2(0, player.gravDir * player.gravity); }

                // Prolong mid-slash player animation
                RecentreSlash(projectile, player);
                if (player.itemAnimation <= player.itemAnimationMax - freezeFrame) { player.itemAnimation = player.itemAnimationMax - freezeFrame; }
                ...
                if (dashEndVelocity == null) {
                    ...
                    if (speed < player.maxFallSpeed) { player.velocity = projVel * speed; } else { player.velocity = projVel * player.maxFallSpeed; }
        ...
        public static void NormalSlash(Projectile projectile, Player player) {
            if (projectile.ai[0] <= 0f) {
                ...
                if (Math.Abs(projectile.rotation) == HALFPI) { projectile.spriteDirection = player.direction; } else { projectile.spriteDirection = Math.Abs(projectile.rotation) <= HALFPI ? 1 : -1; }
        ...
        public static bool PreDrawSlashAndWeapon(SpriteBatch spriteBatch, Projectile projectile, int weaponItemID, Color weaponColor, Texture2D slashTexture, Color slashColor, int slashFramecount, float slashDirection = 1f, bool shadow = false) {
            Player player = Main.player[projectile.owner];
            Texture2D weapon = (Texture2D)Terraria.GameContent.TextureAssets.Item[weaponItemID];
            if (slashTexture == null) {
                slashTexture = (Texture2D)Terraria.GameContent.TextureAssets.Projectile[projectile.type];
